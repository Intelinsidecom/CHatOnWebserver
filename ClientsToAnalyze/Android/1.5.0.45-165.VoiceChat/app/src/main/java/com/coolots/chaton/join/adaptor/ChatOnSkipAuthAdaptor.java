...
import com.coolots.p2pmsg.model.ChatOnSkipAuthAsk;
import com.coolots.p2pmsg.model.ChatOnSkipAuthRep;
...
public class ChatOnSkipAuthAdaptor extends HttpAdaptor {
    public static final String CLASSNAME = "[ChatOnSkipAuthAdaptor]";
    ...
    public ChatOnSkipAuthAdaptor(PhoneNoAuthInfo phoneNoAuthInfo, String guid, Handler handler) {
        super(MessageInfo.ChatOnSkipAuthAsk, null, handler, HttpAdaptorTimeOutValue.HTTP_CONNECTION_TIMEOUT_CHATONSMSAUTH);
        ChatOnSkipAuthAsk ask = new ChatOnSkipAuthAsk();
        ask.setSamsungAccountID(guid);
    ...
    protected void printAdaptorKind() {
        logE("ADAPTOR_TRACE ChatOnSkipAuthAdaptor ChatOnSkipAuthAsk");
    }

    @Override // com.sds.coolots.common.httpAdaptor.HttpAdaptor
    protected void processRecvMessage(P2PMsg p2pMsg) {
        if (p2pMsg.getMsgBody() instanceof ChatOnSkipAuthRep) {
            ChatOnSkipAuthRep rep = (ChatOnSkipAuthRep) p2pMsg.getMsgBody();
            this.mPhoneNoAuthInfo.setAuthID(rep.getUserToken());
            if ("1".equals(rep.getResult())) {
                this.mPhoneNoAuthInfo.setResult(true);
            } else {
                this.mPhoneNoAuthInfo.setResult(false);
            }
