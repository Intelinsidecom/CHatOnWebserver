...
public class Marker implements SpenPenInterface {
    ...
    public final int nativeMarker = native_init();
    ...
    public void setProperty(Bundle propertyMap) {
        native_setProperty(this.nativeMarker, propertyMap);
    }

    @Override // com.samsung.android.sdk.pen.plugin.interfaces.SpenPluginInterface
    public void getProperty(Bundle propertyMap) {
        native_getProperty(this.nativeMarker, propertyMap);
    }
    ...
    public void draw(MotionEvent event, RectF rect) {
        if (this.mBitmap != null) {
            ...
            if (!native_draw(this.nativeMarker, event, rect, event.getToolType(0))) {
    ...
    public void redrawPen(MotionEvent allEvent, RectF rect) {
        if (this.mBitmap != null) {
            ...
            this.mBitmap.setPixel(0, 0, 0);
            if (!native_redraw(this.nativeMarker, allEvent, rect, allEvent.getToolType(0))) {
                SpenError.ThrowUncheckedException(SpenError.getError(), toString());
            }
        }
    }
    ...
    public void setBitmap(Bitmap bitmap) {
        this.mBitmap = bitmap;
        if (this.mBitmap != null && !native_setBitmap(this.nativeMarker, this.mBitmap)) {
            SpenError.ThrowUncheckedException(SpenError.getError(), toString());
        }
    }
    ...
    public void setSize(float size) {
        if (!native_setSize(this.nativeMarker, size)) {
            SpenError.ThrowUncheckedException(SpenError.getError(), toString());
        }
    }
    ...
    public void setColor(int color) {
        if (!native_setColor(this.nativeMarker, color)) {
            SpenError.ThrowUncheckedException(SpenError.getError(), toString());
        }
    }
    ...
    public void setCurveEnabled(boolean curve) {
        if (!native_setCurveEnabled(this.nativeMarker, curve)) {
            SpenError.ThrowUncheckedException(SpenError.getError(), toString());
        }
    }
    ...
    public void setAdvancedSetting(String advancedSetting) {
        if (!native_setAdvancedSetting(this.nativeMarker, advancedSetting)) {
            SpenError.ThrowUncheckedException(SpenError.getError(), toString());
        }
    }
    ...
    public RectF getStrokeRect(PointF[] points, float[] pressures, int[] timestamps, float size, boolean isCurvable, String advanced) {
        return native_getStrokeRect(this.nativeMarker, points, pressures, timestamps, size, isCurvable, advanced);
    }

    @Override // com.samsung.android.sdk.pen.plugin.interfaces.SpenPenInterface
    public boolean getPenAttribute(int attribute) {
        return native_getPenAttribute(this.nativeMarker, attribute);
    }
}
