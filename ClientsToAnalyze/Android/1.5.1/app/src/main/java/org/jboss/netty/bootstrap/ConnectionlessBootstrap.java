...
public class ConnectionlessBootstrap extends Bootstrap {
    ...
    public Channel bind(SocketAddress socketAddress) {
        ...
        try {
            ...
            channelNewChannel.getConfig().setPipelineFactory(getPipelineFactory());
            channelNewChannel.getConfig().setOptions(getOptions());
            ChannelFuture channelFutureBind = channelNewChannel.bind(socketAddress);
            channelFutureBind.awaitUninterruptibly();
            if (channelFutureBind.isSuccess()) {
                return channelNewChannel;
            }
            channelFutureBind.getChannel().close().awaitUninterruptibly();
            throw new ChannelException("Failed to bind to: " + socketAddress, channelFutureBind.getCause());
        } catch (Exception e) {
