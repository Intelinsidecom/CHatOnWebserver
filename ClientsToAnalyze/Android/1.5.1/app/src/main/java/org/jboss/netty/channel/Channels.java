...
public class Channels {
    ...
    public static ChannelPipeline pipeline(ChannelPipeline channelPipeline) {
        ...
        for (Map.Entry entry : channelPipeline.toMap().entrySet()) {
            channelPipelinePipeline.addLast((String) entry.getKey(), (ChannelHandler) entry.getValue());
        }
    ...
    public static ChannelPipeline pipeline(ChannelHandler... channelHandlerArr) {
        ...
        for (int i = 0; i < channelHandlerArr.length && (channelHandler = channelHandlerArr[i]) != null; i++) {
            channelPipelinePipeline.addLast(ConversionUtil.toString(i), channelHandler);
        }
        return channelPipelinePipeline;
    }
    ...
    public static ChannelFuture setInterestOps(Channel channel, int i) {
        ...
        int iFilterDownstreamInterestOps = filterDownstreamInterestOps(i);
        ChannelFuture channelFutureFuture = future(channel);
        channel.getPipeline().sendDownstream(new DownstreamChannelStateEvent(channel, channelFutureFuture, ChannelState.INTEREST_OPS, Integer.valueOf(iFilterDownstreamInterestOps)));
        return channelFutureFuture;
    }
    ...
    public static ChannelFuture unbind(Channel channel) {
        ChannelFuture channelFutureFuture = future(channel);
        channel.getPipeline().sendDownstream(new DownstreamChannelStateEvent(channel, channelFutureFuture, ChannelState.BOUND, null));
        return channelFutureFuture;
    }
