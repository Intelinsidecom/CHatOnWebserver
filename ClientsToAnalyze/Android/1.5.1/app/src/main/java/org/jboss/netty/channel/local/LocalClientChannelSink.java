    ...
    public void eventSunk(ChannelPipeline channelPipeline, ChannelEvent channelEvent) {
        if (!(channelEvent instanceof ChannelStateEvent)) {
            if (channelEvent instanceof MessageEvent) {
                ...
                DefaultLocalChannel defaultLocalChannel = (DefaultLocalChannel) messageEvent.getChannel();
                boolean zOffer = defaultLocalChannel.writeBuffer.offer(messageEvent);
        ...
        DefaultLocalChannel defaultLocalChannel2 = (DefaultLocalChannel) channelStateEvent.getChannel();
        ...
        switch (state) {
            case OPEN:
                if (Boolean.FALSE.equals(value)) {
                    defaultLocalChannel2.closeNow(future);
                    return;
                }
                return;
            case BOUND:
                if (value != null) {
                    bind(defaultLocalChannel2, future, (LocalAddress) value);
                    return;
                } else {
                    defaultLocalChannel2.closeNow(future);
                    return;
            ...
            case CONNECTED:
                if (value != null) {
                    connect(defaultLocalChannel2, future, (LocalAddress) value);
                    return;
                } else {
                    defaultLocalChannel2.closeNow(future);
                    return;
