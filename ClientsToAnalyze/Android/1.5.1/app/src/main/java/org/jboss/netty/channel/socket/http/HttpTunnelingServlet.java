...
public class HttpTunnelingServlet extends HttpServlet {
    ...
    private static ChannelBuffer read(PushbackInputStream pushbackInputStream) throws IOException {
        ...
        if (iAvailable > 0) {
            ...
            bArr = bArr2;
            i = pushbackInputStream.read(bArr2);
        } else {
            if (iAvailable != 0) {
                return null;
            }
            int i2 = pushbackInputStream.read();
            if (i2 < 0 || pushbackInputStream.available() < 0) {
                return null;
            }
            pushbackInputStream.unread(i2);
            byte[] bArr3 = new byte[pushbackInputStream.available()];
            bArr = bArr3;
            i = pushbackInputStream.read(bArr3);
        }
        if ($assertionsDisabled || i > 0) {
            return i == bArr.length ? ChannelBuffers.wrappedBuffer(bArr) : ChannelBuffers.wrappedBuffer(bArr, 0, i);
        }
