...
public class HttpHeaders {
    ...
    public final class Names {
        ...
        /* renamed from: TE */
        public static final String f4576TE = "TE";
    ...
    /* renamed from: eq */
    private static boolean m4391eq(String str, String str2) {
    ...
    public static boolean is100ContinueExpected(HttpMessage httpMessage) {
        String header;
        if ((httpMessage instanceof HttpRequest) && httpMessage.getProtocolVersion().compareTo(HttpVersion.HTTP_1_1) >= 0 && (header = httpMessage.getHeader(Names.EXPECT)) != null) {
            if ("100-continue".equalsIgnoreCase(header)) {
                return true;
            }
            Iterator it = httpMessage.getHeaders(Names.EXPECT).iterator();
            while (it.hasNext()) {
                if ("100-continue".equalsIgnoreCase((String) it.next())) {
                    return true;
                }
            }
            return false;
    ...
    public static boolean isKeepAlive(HttpMessage httpMessage) {
        ...
        return httpMessage.getProtocolVersion().isKeepAliveDefault() ? !"close".equalsIgnoreCase(header) : "keep-alive".equalsIgnoreCase(header);
    }
    ...
    private void removeHeader0(int i, int i2, String str) {
        ...
        while (entry.hash == i && m4391eq(str, entry.key)) {
            entry.remove();
            entry = entry.next;
            if (entry == null) {
        ...
        while (true) {
            ...
            if (entry2.hash == i && m4391eq(str, entry2.key)) {
    ...
    public static void set100ContinueExpected(HttpMessage httpMessage, boolean z) {
        if (z) {
            httpMessage.setHeader(Names.EXPECT, "100-continue");
        } else {
            httpMessage.removeHeader(Names.EXPECT);
        }
    }
    ...
    public static void setIntHeader(HttpMessage httpMessage, String str, Iterable iterable) {
        httpMessage.setHeader(str, iterable);
    }
    ...
    void addHeader(String str, Object obj) {
        ...
        HttpCodecUtil.validateHeaderValue(string);
        int iHash = hash(str);
        addHeader0(iHash, index(iHash), str, string);
    }
    ...
    String getHeader(String str) {
        ...
        int iHash = hash(str);
        for (Entry entry = this.entries[index(iHash)]; entry != null; entry = entry.next) {
            if (entry.hash == iHash && m4391eq(str, entry.key)) {
                return entry.value;
            }
        }
        return null;
    ...
    void removeHeader(String str) {
        ...
        int iHash = hash(str);
        removeHeader0(iHash, index(iHash), str);
    }
    ...
    void setHeader(String str, Iterable iterable) {
        ...
        validateHeaderName(str);
        int iHash = hash(str);
        int iIndex = index(iHash);
        removeHeader0(iHash, iIndex, str);
        Iterator it = iterable.iterator();
        while (it.hasNext() && (next = it.next()) != null) {
            String string = toString(next);
            HttpCodecUtil.validateHeaderValue(string);
            addHeader0(iHash, iIndex, str, string);
        }
    }
