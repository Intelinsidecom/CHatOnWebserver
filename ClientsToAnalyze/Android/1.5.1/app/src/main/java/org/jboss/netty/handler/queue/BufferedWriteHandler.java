...
public class BufferedWriteHandler extends SimpleChannelHandler {
    ...
    public BufferedWriteHandler(Queue queue, boolean z) {
        if (queue == null) {
    ...
    private List consolidatedWrite(final List list) {
        int size = list.size();
        if (size == 1) {
        ...
        for (int i = 0; i < channelBufferArr.length; i++) {
            channelBufferArr[i] = (ChannelBuffer) ((MessageEvent) list.get(i)).getMessage();
        }
    ...
    public void flush(boolean z) {
        ...
        if (!z) {
            synchronized (this) {
                while (true) {
                    MessageEvent messageEvent = (MessageEvent) queue.poll();
        ...
        synchronized (this) {
            List listConsolidatedWrite = arrayList;
            while (true) {
                MessageEvent messageEvent2 = (MessageEvent) queue.poll();
                if (messageEvent2 == null) {
                    consolidatedWrite(listConsolidatedWrite);
                } else {
                    if (messageEvent2.getMessage() instanceof ChannelBuffer) {
                        listConsolidatedWrite.add(messageEvent2);
                    } else {
                        listConsolidatedWrite = consolidatedWrite(listConsolidatedWrite);
                        if (listConsolidatedWrite == null) {
                            listConsolidatedWrite = new ArrayList();
                        }
                        channelHandlerContext.sendDownstream(messageEvent2);
                    }
                    listConsolidatedWrite = listConsolidatedWrite;
                }
            }
        }
    }
