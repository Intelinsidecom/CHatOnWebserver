...
public class DefaultObjectSizeEstimator implements ObjectSizeEstimator {
    private final ConcurrentMap class2size = new ConcurrentIdentityWeakKeyHashMap();
    ...
    private static int align(int i) {
        int i2 = i % 8;
        return i2 != 0 ? (8 - i2) + i : i;
    }

    private int estimateSize(Class cls, Set set) {
        Set hashSet;
        ...
        while (superclass != null) {
            ...
            for (Field field : superclass.getDeclaredFields()) {
                if ((field.getModifiers() & 8) == 0) {
                    iEstimateSize += estimateSize(field.getType(), hashSet);
                }
            }
