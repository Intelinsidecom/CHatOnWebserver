...
public class HashedWheelTimer implements Timer {
    ...
    /* renamed from: id */
    private static final AtomicInteger f4587id = new AtomicInteger();
    ...
    final class Worker implements Runnable {
        ...
        private void fetchExpiredTimeouts(List list, ReusableIterator reusableIterator, long j) {
            ArrayList<HashedWheelTimeout> arrayList;
    ...
    public HashedWheelTimer(ThreadFactory threadFactory, long j, TimeUnit timeUnit, int i) {
        ...
        this.workerThread = threadFactory.newThread(new ThreadRenamingRunnable(this.worker, "Hashed wheel timer #" + f4587id.incrementAndGet()));
    ...
    void scheduleTimeout(HashedWheelTimeout hashedWheelTimeout, long j) {
        long j2 = j < this.tickDuration ? this.tickDuration : j;
        long j3 = ((j2 % this.roundDuration) / this.tickDuration) + (j2 % this.tickDuration != 0 ? 1 : 0);
        long j4 = (j2 / this.roundDuration) - (j2 % this.roundDuration == 0 ? 1 : 0);
        this.lock.readLock().lock();
        try {
            int i = (int) ((j3 + this.wheelCursor) & this.mask);
            hashedWheelTimeout.stopIndex = i;
            hashedWheelTimeout.remainingRounds = j4;
