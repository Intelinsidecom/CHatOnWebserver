...
final class BackStackRecord extends FragmentTransaction implements FragmentManager.BackStackEntry, Runnable {
    ...
    public void dump(String str, FileDescriptor fileDescriptor, PrintWriter printWriter, String[] strArr) {
        ...
        if (this.mHead != null) {
            ...
            String str2 = str + "    ";
            for (C0010Op c0010Op = this.mHead; c0010Op != null; c0010Op = c0010Op.next) {
                printWriter.print(str);
                ...
                printWriter.print(c0010Op.cmd);
                printWriter.print(" fragment=");
                printWriter.println(c0010Op.fragment);
                if (c0010Op.enterAnim != 0 || c0010Op.exitAnim != 0) {
                    printWriter.print(str);
                    printWriter.print("enterAnim=#");
                    printWriter.print(Integer.toHexString(c0010Op.enterAnim));
                    printWriter.print(" exitAnim=#");
                    printWriter.println(Integer.toHexString(c0010Op.exitAnim));
                }
                if (c0010Op.popEnterAnim != 0 || c0010Op.popExitAnim != 0) {
                    printWriter.print(str);
                    printWriter.print("popEnterAnim=#");
                    printWriter.print(Integer.toHexString(c0010Op.popEnterAnim));
                    printWriter.print(" popExitAnim=#");
                    printWriter.println(Integer.toHexString(c0010Op.popExitAnim));
                }
                if (c0010Op.removed != null && c0010Op.removed.size() > 0) {
                    for (int i = 0; i < c0010Op.removed.size(); i++) {
                        printWriter.print(str2);
                        if (c0010Op.removed.size() == 1) {
                        ...
                        printWriter.println(c0010Op.removed.get(i));
                    }
                }
            }
        }
    ...
    void addOp(C0010Op c0010Op) {
        if (this.mHead == null) {
            this.mTail = c0010Op;
            this.mHead = c0010Op;
        } else {
            c0010Op.prev = this.mTail;
            this.mTail.next = c0010Op;
            this.mTail = c0010Op;
        }
        c0010Op.enterAnim = this.mEnterAnim;
        c0010Op.exitAnim = this.mExitAnim;
        c0010Op.popEnterAnim = this.mPopEnterAnim;
        c0010Op.popExitAnim = this.mPopExitAnim;
        this.mNumOp++;
    }
    ...
    private void doAddOp(int i, Fragment fragment, String str, int i2) {
        ...
        C0010Op c0010Op = new C0010Op();
        c0010Op.cmd = i2;
        c0010Op.fragment = fragment;
        addOp(c0010Op);
    }
    ...
    void bumpBackStackNesting(int i) {
        if (this.mAddToBackStack) {
            ...
            for (C0010Op c0010Op = this.mHead; c0010Op != null; c0010Op = c0010Op.next) {
                c0010Op.fragment.mBackStackNesting += i;
                if (FragmentManagerImpl.DEBUG) {
                    Log.v(TAG, "Bump nesting of " + c0010Op.fragment + " to " + c0010Op.fragment.mBackStackNesting);
                }
                if (c0010Op.removed != null) {
                    for (int size = c0010Op.removed.size() - 1; size >= 0; size--) {
                        Fragment fragment = (Fragment) c0010Op.removed.get(size);
    ...
    public void run() {
        ...
        for (C0010Op c0010Op = this.mHead; c0010Op != null; c0010Op = c0010Op.next) {
            switch (c0010Op.cmd) {
                case 1:
                    Fragment fragment = c0010Op.fragment;
                    fragment.mNextAnim = c0010Op.enterAnim;
                    this.mManager.addFragment(fragment, false);
                    break;
                case 2:
                    Fragment fragment2 = c0010Op.fragment;
                    if (this.mManager.mAdded != null) {
                        for (int i = 0; i < this.mManager.mAdded.size(); i++) {
                            ...
                            if (fragment3.mContainerId == fragment2.mContainerId) {
                                if (c0010Op.removed == null) {
                                    c0010Op.removed = new ArrayList();
                                }
                                c0010Op.removed.add(fragment3);
                                fragment3.mNextAnim = c0010Op.exitAnim;
                                if (this.mAddToBackStack) {
                    ...
                    fragment2.mNextAnim = c0010Op.enterAnim;
                    this.mManager.addFragment(fragment2, false);
                    break;
                case 3:
                    Fragment fragment4 = c0010Op.fragment;
                    fragment4.mNextAnim = c0010Op.exitAnim;
                    this.mManager.removeFragment(fragment4, this.mTransition, this.mTransitionStyle);
                    break;
                case 4:
                    Fragment fragment5 = c0010Op.fragment;
                    fragment5.mNextAnim = c0010Op.exitAnim;
                    this.mManager.hideFragment(fragment5, this.mTransition, this.mTransitionStyle);
                    break;
                case 5:
                    Fragment fragment6 = c0010Op.fragment;
                    fragment6.mNextAnim = c0010Op.enterAnim;
                    this.mManager.showFragment(fragment6, this.mTransition, this.mTransitionStyle);
                    break;
                case 6:
                    Fragment fragment7 = c0010Op.fragment;
                    fragment7.mNextAnim = c0010Op.exitAnim;
                    this.mManager.detachFragment(fragment7, this.mTransition, this.mTransitionStyle);
                    break;
                case 7:
                    Fragment fragment8 = c0010Op.fragment;
                    fragment8.mNextAnim = c0010Op.enterAnim;
                    this.mManager.attachFragment(fragment8, this.mTransition, this.mTransitionStyle);
                    break;
                default:
                    throw new IllegalArgumentException("Unknown cmd: " + c0010Op.cmd);
            }
        }
    ...
    public void popFromBackStack(boolean z) {
        ...
        bumpBackStackNesting(-1);
        for (C0010Op c0010Op = this.mTail; c0010Op != null; c0010Op = c0010Op.prev) {
            switch (c0010Op.cmd) {
                case 1:
                    Fragment fragment = c0010Op.fragment;
                    fragment.mNextAnim = c0010Op.popExitAnim;
                    this.mManager.removeFragment(fragment, FragmentManagerImpl.reverseTransit(this.mTransition), this.mTransitionStyle);
                    break;
                case 2:
                    Fragment fragment2 = c0010Op.fragment;
                    fragment2.mNextAnim = c0010Op.popExitAnim;
                    this.mManager.removeFragment(fragment2, FragmentManagerImpl.reverseTransit(this.mTransition), this.mTransitionStyle);
                    if (c0010Op.removed != null) {
                        for (int i = 0; i < c0010Op.removed.size(); i++) {
                            Fragment fragment3 = (Fragment) c0010Op.removed.get(i);
                            fragment3.mNextAnim = c0010Op.popEnterAnim;
