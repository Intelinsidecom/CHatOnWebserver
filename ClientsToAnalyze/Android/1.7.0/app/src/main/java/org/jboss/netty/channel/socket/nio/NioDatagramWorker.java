        ...
        synchronized (this.startStopLock) {
            if (!this.started) {
                try {
                    ...
                    try {
                        this.executor.execute(new ThreadRenamingRunnable(this, "New I/O datagram worker #" + this.bossId + "'-'" + this.f5794id));
                    } finally {
                    }
                } catch (Throwable th) {
        ...
        synchronized (nioDatagramChannel.writeLock) {
            ...
            while (true) {
                ...
                if (messageEvent2 == null) {
                    ...
                    nioDatagramChannel.currentWriteEvent = messageEvent3;
                    if (messageEvent3 == null) {
                        z = true;
                        nioDatagramChannel.writeSuspended = false;
                        break;
                    } else {
                        sendBufferAcquire = socketSendBufferPool.acquire(messageEvent3.getMessage());
        ...
        synchronized (nioDatagramChannel.writeLock) {
            ...
            if (!queue.isEmpty()) {
                ...
                while (true) {
                    ...
                    messageEvent2.getFuture().setFailure(closedChannelException);
                    z = true;
                }
                z2 = z;
