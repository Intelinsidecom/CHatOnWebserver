...
public class ZlibEncoder extends OneToOneEncoder implements LifeCycleAwareChannelHandler {
    ...
    public ZlibEncoder(ZlibWrapper zlibWrapper, int i) {
        ...
        synchronized (this.f5800z) {
            int iDeflateInit = this.f5800z.deflateInit(i, ZlibUtil.convertWrapperType(zlibWrapper));
            if (iDeflateInit != 0) {
                ZlibUtil.fail(this.f5800z, "initialization failure", iDeflateInit);
            }
        }
    }
    ...
    public ZlibEncoder(int i, byte[] bArr) {
        ...
        synchronized (this.f5800z) {
            int iDeflateInit = this.f5800z.deflateInit(i, JZlib.W_ZLIB);
            if (iDeflateInit != 0) {
                ZlibUtil.fail(this.f5800z, "initialization failure", iDeflateInit);
            } else {
                int iDeflateSetDictionary = this.f5800z.deflateSetDictionary(bArr, bArr.length);
                if (iDeflateSetDictionary != 0) {
                    ZlibUtil.fail(this.f5800z, "failed to set the dictionary", iDeflateSetDictionary);
                }
            }
        }
    }
        ...
        if ((obj instanceof ChannelBuffer) && !this.finished.get()) {
            synchronized (this.f5800z) {
                try {
                    ...
                    this.f5800z.next_in = bArr;
                    this.f5800z.next_in_index = 0;
                    this.f5800z.avail_in = bArr.length;
                    byte[] bArr2 = new byte[((int) Math.ceil(bArr.length * 1.001d)) + 12];
                    this.f5800z.next_out = bArr2;
                    this.f5800z.next_out_index = 0;
                    this.f5800z.avail_out = bArr2.length;
                    int iDeflate = this.f5800z.deflate(2);
                    if (iDeflate != 0) {
                        ZlibUtil.fail(this.f5800z, "compression failure", iDeflate);
                    }
                    if (this.f5800z.next_out_index != 0) {
                        obj = channelHandlerContext.getChannel().getConfig().getBufferFactory().getBuffer(channelBuffer.order(), bArr2, 0, this.f5800z.next_out_index);
                    } else {
                        obj = ChannelBuffers.EMPTY_BUFFER;
                    }
                } finally {
                    this.f5800z.next_in = null;
                    this.f5800z.next_out = null;
                }
            }
        }
        return obj;
    ...
    private ChannelFuture finishEncode(final ChannelHandlerContext channelHandlerContext, final ChannelEvent channelEvent) {
        ...
        synchronized (this.f5800z) {
            try {
                this.f5800z.next_in = EMPTY_ARRAY;
                this.f5800z.next_in_index = 0;
                this.f5800z.avail_in = 0;
                byte[] bArr = new byte[32];
                this.f5800z.next_out = bArr;
                this.f5800z.next_out_index = 0;
                this.f5800z.avail_out = bArr.length;
                int iDeflate = this.f5800z.deflate(4);
                if (iDeflate != 0 && iDeflate != 1) {
                    channelFutureFuture = Channels.failedFuture(channelHandlerContext.getChannel(), ZlibUtil.exception(this.f5800z, "compression failure", iDeflate));
                } else if (this.f5800z.next_out_index != 0) {
                    channelFutureFuture = Channels.future(channelHandlerContext.getChannel());
                    buffer = channelHandlerContext.getChannel().getConfig().getBufferFactory().getBuffer(bArr, 0, this.f5800z.next_out_index);
                } else {
                    channelFutureFuture = Channels.future(channelHandlerContext.getChannel());
                    buffer = ChannelBuffers.EMPTY_BUFFER;
                }
            } finally {
                this.f5800z.deflateEnd();
                this.f5800z.next_in = null;
                this.f5800z.next_out = null;
            }
        }
