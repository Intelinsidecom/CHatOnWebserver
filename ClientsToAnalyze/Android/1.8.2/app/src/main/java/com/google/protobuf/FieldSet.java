...
final class FieldSet {
    ...
    private void mergeFromField(Map.Entry entry) {
        ...
        if (fieldDescriptorLite.getLiteJavaType() == WireFormat.JavaType.MESSAGE) {
            ...
            if (obj2 == null) {
                ...
                this.fields.put((Comparable) fieldDescriptorLite, (Object) fieldDescriptorLite.internalMergeFrom(((MessageLite) obj2).toBuilder(), (MessageLite) value).build());
                return;
            }
        }
        this.fields.put((Comparable) fieldDescriptorLite, value);
    ...
    public static Object readPrimitiveField(CodedInputStream codedInputStream, WireFormat.FieldType fieldType) {
        switch (C00441.$SwitchMap$com$google$protobuf$WireFormat$FieldType[fieldType.ordinal()]) {
            case 1:
    ...
    private int getMessageSetSerializedSize(Map.Entry entry) {
        FieldDescriptorLite fieldDescriptorLite = (FieldDescriptorLite) entry.getKey();
        return (fieldDescriptorLite.getLiteJavaType() != WireFormat.JavaType.MESSAGE || fieldDescriptorLite.isRepeated() || fieldDescriptorLite.isPacked()) ? computeFieldSize(fieldDescriptorLite, entry.getValue()) : CodedOutputStream.computeMessageSetExtensionSize(((FieldDescriptorLite) entry.getKey()).getNumber(), (MessageLite) entry.getValue());
    }
