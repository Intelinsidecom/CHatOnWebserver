package com.sec.chaton.multimedia.vcard;

import com.sec.chaton.util.C1786r;
import java.io.UnsupportedEncodingException;
import java.math.BigInteger;

/* compiled from: VCardParser.java */
/* renamed from: com.sec.chaton.multimedia.vcard.r */
/* loaded from: classes.dex */
public class C1048r {

    /* renamed from: c */
    private boolean f3902c = false;

    /* renamed from: b */
    boolean f3901b = true;

    /* renamed from: a */
    C1038h f3900a = new C1038h();

    /* renamed from: a */
    public C1038h m4044a(String str) {
        String[] strArrM4049d;
        int i = 0;
        for (String str2 : str.split("\n")) {
            C1786r.m6064d("RAW LINE : " + str2);
            if (str2.startsWith("BEGIN:VCARD")) {
                i++;
                if (i > 1 && !this.f3902c) {
                    this.f3902c = true;
                }
            } else if (str2.startsWith("NOTE")) {
                String strM4050e = m4050e(str2);
                if (strM4050e != null) {
                    this.f3900a.f3846d = strM4050e;
                }
            } else if (str2.startsWith("N") || str2.startsWith("FN")) {
                if (!str2.startsWith("NICK") && !str2.startsWith("NOTE")) {
                    this.f3900a.f3843a = m4047b(str2);
                }
            } else if (str2.startsWith("TEL")) {
                String[] strArrM4048c = m4048c(str2);
                if (strArrM4048c != null) {
                    this.f3900a.m4027a(Integer.parseInt(strArrM4048c[2]), strArrM4048c[0], strArrM4048c[1], this.f3901b);
                    this.f3901b = false;
                }
            } else if (str2.startsWith("EMAIL") && (strArrM4049d = m4049d(str2)) != null) {
                this.f3900a.m4028b(Integer.parseInt(strArrM4049d[2]), strArrM4049d[0], strArrM4049d[1], this.f3901b);
            }
        }
        return this.f3900a;
    }

    /* renamed from: a */
    public boolean m4046a() {
        return this.f3902c;
    }

    /* renamed from: b */
    public String m4047b(String str) {
        String[] strArrSplit = str.split(":");
        if (strArrSplit.length < 2) {
            return null;
        }
        String str2 = strArrSplit[1];
        String strSubstring = "";
        for (String str3 : strArrSplit[0].split(";")) {
            if (str3.startsWith("CHARSET")) {
                strSubstring = str3.substring(8, str3.length());
            }
        }
        return m4045a(str2, strSubstring);
    }

    /* renamed from: c */
    public String[] m4048c(String str) {
        int i = 0;
        String[] strArr = new String[3];
        String[] strArrSplit = str.split(":");
        if (strArrSplit.length < 2) {
            return null;
        }
        strArr[0] = strArrSplit[1];
        String[] strArrSplit2 = strArrSplit[0].split(";");
        for (String str2 : strArrSplit2) {
            if ("HOME".equals(str2)) {
                strArr[1] = str2;
                i = 1;
            } else if ("CELL".equals(str2)) {
                strArr[1] = str2;
                i = 2;
            } else if ("WORK".equals(str2)) {
                strArr[1] = str2;
                i = 3;
            } else if ("WORK-FAX".equals(str2)) {
                i = 4;
                strArr[1] = str2;
            } else if ("HOME-FAX".equals(str2)) {
                i = 5;
                strArr[1] = str2;
            } else if ("PAGER".equals(str2)) {
                i = 6;
                strArr[1] = str2;
            } else if ("OTHER".equals(str2)) {
                i = 7;
                strArr[1] = str2;
            } else if ("CALLBACK".equals(str2)) {
                i = 8;
                strArr[1] = str2;
            } else if ("CAR".equals(str2)) {
                i = 9;
                strArr[1] = str2;
            } else if ("COMPANY_MAIN".equals(str2)) {
                i = 10;
                strArr[1] = str2;
            } else if ("ISDN".equals(str2)) {
                i = 11;
                strArr[1] = str2;
            } else if ("MAIN".equals(str2)) {
                i = 12;
                strArr[1] = str2;
            } else if ("OTHER_FAX".equals(str2)) {
                i = 13;
                strArr[1] = str2;
            } else if ("RADIO".equals(str2)) {
                i = 14;
                strArr[1] = str2;
            } else if ("TELEX".equals(str2)) {
                i = 15;
                strArr[1] = str2;
            } else if ("TTY_TDD".equals(str2)) {
                i = 16;
                strArr[1] = str2;
            } else if ("WORK_MOBILE".equals(str2)) {
                i = 17;
                strArr[1] = str2;
            } else if ("WORK_PAGER".equals(str2)) {
                i = 18;
                strArr[1] = str2;
            } else if ("ASSISTANT".equals(str2)) {
                i = 19;
                strArr[1] = str2;
            } else if ("MMS".equals(str2)) {
                i = 20;
                strArr[1] = str2;
            }
            strArr[2] = String.valueOf(i);
        }
        return strArr;
    }

    /* renamed from: d */
    public String[] m4049d(String str) {
        String strSubstring;
        String[] strArr = new String[3];
        String[] strArrSplit = str.split(":");
        if (strArrSplit.length < 2) {
            return null;
        }
        String str2 = strArrSplit[1];
        String[] strArrSplit2 = strArrSplit[0].split(";");
        String str3 = "";
        int length = strArrSplit2.length;
        int i = 0;
        int i2 = 0;
        String str4 = "OTHER";
        while (i < length) {
            String str5 = strArrSplit2[i];
            if (str5.startsWith("CHARSET")) {
                strSubstring = str5.substring(8, str5.length());
            } else if ("HOME".equals(str5)) {
                i2 = 1;
                str4 = str5;
                strSubstring = str3;
            } else if ("WORK".equals(str5)) {
                i2 = 2;
                str4 = str5;
                strSubstring = str3;
            } else if ("OTHER".equals(str5)) {
                i2 = 3;
                str4 = str5;
                strSubstring = str3;
            } else if ("MOBILE".equals(str5)) {
                i2 = 4;
                str4 = str5;
                strSubstring = str3;
            } else if (str5.startsWith("X-")) {
                i2 = 3;
                str4 = null;
                strSubstring = str3;
            } else {
                strSubstring = str3;
            }
            i++;
            str3 = strSubstring;
        }
        strArr[0] = m4045a(str2, str3);
        strArr[1] = str4;
        strArr[2] = String.valueOf(i2);
        return strArr;
    }

    /* renamed from: e */
    public String m4050e(String str) {
        String str2 = "";
        String[] strArrSplit = str.split(":");
        if (strArrSplit.length < 2) {
            return null;
        }
        String str3 = strArrSplit[1];
        String[] strArrSplit2 = strArrSplit[0].split(";");
        int length = strArrSplit2.length;
        int i = 0;
        while (i < length) {
            String str4 = strArrSplit2[i];
            if (!str4.startsWith("CHARSET")) {
                str4 = str2;
            }
            i++;
            str2 = str4;
        }
        return m4045a(str3, str2);
    }

    /* renamed from: a */
    public String m4045a(String str, String str2) {
        String strReplaceAll = str.replaceAll(";", "");
        if (strReplaceAll.contains("=")) {
            String[] strArrSplit = strReplaceAll.split("=");
            StringBuilder sb = new StringBuilder();
            StringBuilder sb2 = new StringBuilder();
            int i = 0;
            while (true) {
                if (i >= strReplaceAll.length()) {
                    i = 0;
                    break;
                }
                if (strReplaceAll.charAt(i) == '=') {
                    break;
                }
                i++;
            }
            int length = strArrSplit.length;
            int i2 = 0;
            int i3 = 0;
            while (i2 < length) {
                String str3 = strArrSplit[i2];
                int i4 = i3 + 1;
                if (i != 0) {
                    sb.append(str3);
                    i = 0;
                } else {
                    if (str3.length() < 2) {
                        if (sb2.length() > 1) {
                            try {
                                sb.append(new String(new BigInteger(sb2.toString(), 16).toByteArray(), str2));
                            } catch (UnsupportedEncodingException e) {
                                e.printStackTrace();
                            }
                            sb2.delete(0, sb2.length());
                        }
                        sb.append(str3);
                    } else if (str3.length() > 2) {
                        sb2.append(str3.substring(0, 2));
                        try {
                            sb.append(new String(new BigInteger(sb2.toString(), 16).toByteArray(), str2).trim());
                        } catch (UnsupportedEncodingException e2) {
                            C1786r.m6054a("Cannot Encode", "VCardParser");
                        }
                        sb2.delete(0, sb2.length());
                        sb.append(str3.substring(2, str3.length()));
                    } else {
                        sb2.append(str3);
                    }
                    if (i4 == strArrSplit.length && sb2.length() > 0) {
                        try {
                            sb.append(new String(new BigInteger(sb2.toString(), 16).toByteArray(), str2).trim());
                        } catch (UnsupportedEncodingException e3) {
                            e3.printStackTrace();
                        }
                    }
                }
                i2++;
                i3 = i4;
            }
            C1786r.m6064d("Decode : " + ((Object) sb));
            return sb.toString();
        }
        C1786r.m6064d(strReplaceAll);
        return strReplaceAll;
    }
}
