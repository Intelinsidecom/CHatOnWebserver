...
class NioServerSocketPipelineSink extends AbstractChannelSink {
    ...
    /* renamed from: id */
    private final int f7856id = nextId.incrementAndGet();
    ...
    NioServerSocketPipelineSink(Executor executor, int i) {
        ...
        for (int i2 = 0; i2 < this.workers.length; i2++) {
            this.workers[i2] = new NioWorker(this.f7856id, i2 + 1, executor);
        }
    }
    ...
    private void bind(NioServerSocketChannel nioServerSocketChannel, ChannelFuture channelFuture, SocketAddress socketAddress) {
        ...
        try {
            try {
                ...
                DeadLockProofWorker.start(((NioServerSocketChannelFactory) nioServerSocketChannel.getFactory()).bossExecutor, new ThreadRenamingRunnable(new Boss(nioServerSocketChannel), "New I/O server boss #" + this.f7856id + " (" + nioServerSocketChannel + ')'));
            } catch (Throwable th) {
