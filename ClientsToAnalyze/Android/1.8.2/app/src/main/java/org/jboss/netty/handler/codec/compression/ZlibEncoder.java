...
public class ZlibEncoder extends OneToOneEncoder implements LifeCycleAwareChannelHandler {
    ...
    public ZlibEncoder(ZlibWrapper zlibWrapper, int i) {
        ...
        synchronized (this.f7861z) {
            int iDeflateInit = this.f7861z.deflateInit(i, ZlibUtil.convertWrapperType(zlibWrapper));
            if (iDeflateInit != 0) {
                ZlibUtil.fail(this.f7861z, "initialization failure", iDeflateInit);
            }
        }
    }
    ...
    public ZlibEncoder(int i, byte[] bArr) {
        ...
        synchronized (this.f7861z) {
            int iDeflateInit = this.f7861z.deflateInit(i, JZlib.W_ZLIB);
            if (iDeflateInit != 0) {
                ZlibUtil.fail(this.f7861z, "initialization failure", iDeflateInit);
            } else {
                int iDeflateSetDictionary = this.f7861z.deflateSetDictionary(bArr, bArr.length);
                if (iDeflateSetDictionary != 0) {
                    ZlibUtil.fail(this.f7861z, "failed to set the dictionary", iDeflateSetDictionary);
                }
            }
        }
    }
        ...
        if ((obj instanceof ChannelBuffer) && !this.finished.get()) {
            synchronized (this.f7861z) {
                try {
                    ...
                    this.f7861z.next_in = bArr;
                    this.f7861z.next_in_index = 0;
                    this.f7861z.avail_in = bArr.length;
                    byte[] bArr2 = new byte[((int) Math.ceil(bArr.length * 1.001d)) + 12];
                    this.f7861z.next_out = bArr2;
                    this.f7861z.next_out_index = 0;
                    this.f7861z.avail_out = bArr2.length;
                    int iDeflate = this.f7861z.deflate(2);
                    if (iDeflate != 0) {
                        ZlibUtil.fail(this.f7861z, "compression failure", iDeflate);
                    }
                    if (this.f7861z.next_out_index != 0) {
                        obj = channelHandlerContext.getChannel().getConfig().getBufferFactory().getBuffer(channelBuffer.order(), bArr2, 0, this.f7861z.next_out_index);
                    } else {
                        obj = ChannelBuffers.EMPTY_BUFFER;
                    }
                } finally {
                    this.f7861z.next_in = null;
                    this.f7861z.next_out = null;
                }
            }
        }
        return obj;
    ...
    private ChannelFuture finishEncode(final ChannelHandlerContext channelHandlerContext, final ChannelEvent channelEvent) {
        ...
        synchronized (this.f7861z) {
            try {
                this.f7861z.next_in = EMPTY_ARRAY;
                this.f7861z.next_in_index = 0;
                this.f7861z.avail_in = 0;
                byte[] bArr = new byte[32];
                this.f7861z.next_out = bArr;
                this.f7861z.next_out_index = 0;
                this.f7861z.avail_out = bArr.length;
                int iDeflate = this.f7861z.deflate(4);
                if (iDeflate != 0 && iDeflate != 1) {
                    channelFutureFuture = Channels.failedFuture(channelHandlerContext.getChannel(), ZlibUtil.exception(this.f7861z, "compression failure", iDeflate));
                } else if (this.f7861z.next_out_index != 0) {
                    channelFutureFuture = Channels.future(channelHandlerContext.getChannel());
                    buffer = channelHandlerContext.getChannel().getConfig().getBufferFactory().getBuffer(bArr, 0, this.f7861z.next_out_index);
                } else {
                    channelFutureFuture = Channels.future(channelHandlerContext.getChannel());
                    buffer = ChannelBuffers.EMPTY_BUFFER;
                }
            } finally {
                this.f7861z.deflateEnd();
                this.f7861z.next_in = null;
                this.f7861z.next_out = null;
            }
        }
