...
final class RelatedResultsJSONImpl extends TwitterResponseImpl implements Serializable, RelatedResults {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if ("Tweet".equals(jSONObject.getString("resultType"))) {
                    ...
                    if (string.length() != 0 && (string.equals(TWEETS_WITH_CONVERSATION) || string.equals(TWEETS_WITH_REPLY) || string.equals(TWEETS_FROM_USER))) {
                        ...
                        ResponseList responseList2 = (ResponseList) this.tweetsMap.get(string);
    ...
    public ResponseList getTweetsWithConversation() {
        ResponseList responseList = (ResponseList) this.tweetsMap.get(TWEETS_WITH_CONVERSATION);
        return responseList != null ? responseList : new ResponseListImpl(0, null);
    }

    @Override // twitter4j.RelatedResults
    public ResponseList getTweetsWithReply() {
        ResponseList responseList = (ResponseList) this.tweetsMap.get(TWEETS_WITH_REPLY);
        return responseList != null ? responseList : new ResponseListImpl(0, null);
    }

    @Override // twitter4j.RelatedResults
    public ResponseList getTweetsFromUser() {
        ResponseList responseList = (ResponseList) this.tweetsMap.get(TWEETS_FROM_USER);
        return responseList != null ? responseList : new ResponseListImpl(0, null);
    }
