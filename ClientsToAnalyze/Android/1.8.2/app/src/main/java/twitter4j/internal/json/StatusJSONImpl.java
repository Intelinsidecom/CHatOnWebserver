...
final class StatusJSONImpl extends TwitterResponseImpl implements Serializable, Status {
    ...
    private void init(JSONObject jSONObject) throws JSONException, TwitterException {
        this.f7903id = z_T4JInternalParseUtil.getLong("id", jSONObject);
    ...
    public int compareTo(Status status) {
        long id = this.f7903id - status.getId();
        if (id < -2147483648L) {
    ...
    static ResponseList createStatusList(HttpResponse httpResponse, Configuration configuration) throws IllegalAccessException, JSONException, TwitterException, IOException, IllegalArgumentException, InvocationTargetException {
        try {
            if (configuration.isJSONStoreEnabled()) {
                DataObjectFactoryUtil.clearThreadLocalMap();
            }
