...
public class OAuth implements Serializable {
    ...
    String generateSignature(String str, OAuthToken oAuthToken) throws IllegalStateException, NoSuchAlgorithmException, InvalidKeyException {
        ...
        byte[] bArrDoFinal = (byte[]) null;
        try {
            Mac mac = Mac.getInstance(HMAC_SHA1);
            if (oAuthToken == null) {
                secretKeySpec = new SecretKeySpec((String.valueOf(encode(this.consumerSecret)) + "&").getBytes(), HMAC_SHA1);
            } else {
                if (oAuthToken.getSecretKeySpec() == null) {
                    oAuthToken.setSecretKeySpec(new SecretKeySpec((String.valueOf(encode(this.consumerSecret)) + "&" + encode(oAuthToken.getTokenSecret())).getBytes(), HMAC_SHA1));
                }
    ...
    public static List toParamList(PostParameter[] postParameterArr) {
    ...
    public static String constructRequestURL(String str) {
        ...
        return String.valueOf(lowerCase) + str.substring(iIndexOf2);
    }
