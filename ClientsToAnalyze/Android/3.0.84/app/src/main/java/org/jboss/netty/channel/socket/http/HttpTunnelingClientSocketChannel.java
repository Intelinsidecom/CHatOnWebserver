...
class HttpTunnelingClientSocketChannel extends AbstractChannel implements SocketChannel {
        ...
        this.realChannel.connect(socketAddress).addListener(new ChannelFutureListener() { // from class: org.jboss.netty.channel.socket.http.HttpTunnelingClientSocketChannel.2
            ...
            public void operationComplete(ChannelFuture channelFuture2) {
                ...
                if (channelFuture2.isSuccess()) {
                    ...
                    ChannelFuture channelFutureHandshake = null;
                    if (sslContext != null) {
                        if (serverName != null) {
                            sSLEngineCreateSSLEngine = sslContext.createSSLEngine(serverName, port);
                        } else {
                            sSLEngineCreateSSLEngine = sslContext.createSSLEngine();
                        }
    ...
    void writeReal(ChannelBuffer channelBuffer, final ChannelFuture channelFuture) {
        ...
        if (i == 0) {
            channelFutureWrite = this.realChannel.write(ChannelBuffers.EMPTY_BUFFER);
        } else {
            channelFutureWrite = this.realChannel.write(new DefaultHttpChunk(channelBuffer));
        }
        channelFutureWrite.addListener(new ChannelFutureListener() { // from class: org.jboss.netty.channel.socket.http.HttpTunnelingClientSocketChannel.3
            @Override // org.jboss.netty.channel.ChannelFutureListener
            public void operationComplete(ChannelFuture channelFuture2) {
                if (channelFuture2.isSuccess()) {
                    channelFuture.setSuccess();
    ...
    private ChannelFuture writeLastChunk() {
        return !this.requestHeaderWritten ? Channels.failedFuture(this, new NotYetConnectedException()) : this.realChannel.write(HttpChunk.LAST_CHUNK);
    }
    ...
    final class ServletChannelHandler extends SimpleChannelUpstreamHandler {
        ...
        public void messageReceived(ChannelHandlerContext channelHandlerContext, MessageEvent messageEvent) {
            if (!this.readingChunks) {
                ...
                if (httpResponse.getStatus().getCode() != HttpResponseStatus.f13704OK.getCode()) {
