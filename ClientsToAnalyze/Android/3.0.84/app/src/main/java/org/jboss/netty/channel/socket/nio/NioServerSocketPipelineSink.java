...
class NioServerSocketPipelineSink extends AbstractChannelSink {
    ...
    /* renamed from: id */
    private final int f13692id = nextId.incrementAndGet();
    ...
    NioServerSocketPipelineSink(Executor executor, int i) {
        ...
        for (int i2 = 0; i2 < this.workers.length; i2++) {
            this.workers[i2] = new NioWorker(this.f13692id, i2 + 1, executor);
        }
    }
    ...
    private void bind(NioServerSocketChannel nioServerSocketChannel, ChannelFuture channelFuture, SocketAddress socketAddress) {
        ...
        try {
            try {
                ...
                DeadLockProofWorker.start(((NioServerSocketChannelFactory) nioServerSocketChannel.getFactory()).bossExecutor, new ThreadRenamingRunnable(new Boss(nioServerSocketChannel), "New I/O server boss #" + this.f13692id + " (" + nioServerSocketChannel + ')'));
            } catch (Throwable th) {
