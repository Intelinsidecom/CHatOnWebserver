...
public class HttpClientCodec implements ChannelDownstreamHandler, ChannelUpstreamHandler {
    ...
    final class Decoder extends HttpResponseDecoder {
        ...
        protected boolean isContentAlwaysEmpty(HttpMessage httpMessage) {
            ...
            HttpMethod httpMethodPoll = HttpClientCodec.this.queue.poll();
            switch (httpMethodPoll.getName().charAt(0)) {
                case 'C':
                    if (code == 200 && HttpMethod.CONNECT.equals(httpMethodPoll)) {
                    ...
                    break;
                case 'H':
                    if (HttpMethod.HEAD.equals(httpMethodPoll)) {
                        return true;
                    }
                    break;
            }
