...
public abstract class HttpContentEncoder extends SimpleChannelHandler {
    ...
    protected abstract EncoderEmbedder<ChannelBuffer> newContentEncoder(String str);
    ...
    public void writeRequested(ChannelHandlerContext channelHandlerContext, MessageEvent messageEvent) {
        ...
        if (message instanceof HttpMessage) {
            ...
            String strPoll = this.acceptEncodingQueue.poll();
            if (strPoll == null) {
                throw new IllegalStateException("cannot send more responses than requests");
            }
            if (httpMessage.isChunked() || httpMessage.getContent().readable()) {
                EncoderEmbedder<ChannelBuffer> encoderEmbedderNewContentEncoder = newContentEncoder(strPoll);
                this.encoder = encoderEmbedderNewContentEncoder;
                if (encoderEmbedderNewContentEncoder != null) {
                    httpMessage.setHeader("Content-Encoding", getTargetContentEncoding(strPoll));
