...
public class MemoryAwareThreadPoolExecutor extends ThreadPoolExecutor {
    ...
    private AtomicLong getChannelCounter(Channel channel) {
        ...
        AtomicLong atomicLongPutIfAbsent = this.channelCounters.get(channel);
        if (atomicLongPutIfAbsent == null && (atomicLongPutIfAbsent = this.channelCounters.putIfAbsent(channel, (atomicLong = new AtomicLong()))) == null) {
            atomicLongPutIfAbsent = atomicLong;
        }
        if (!channel.isOpen()) {
            this.channelCounters.remove(channel);
        }
        return atomicLongPutIfAbsent;
    }
