...
public class OrderedMemoryAwareThreadPoolExecutor extends MemoryAwareThreadPoolExecutor {
    ...
    private Executor getChildExecutor(ChannelEvent channelEvent) {
        ...
        Executor executorPutIfAbsent = this.childExecutors.get(childExecutorKey);
        if (executorPutIfAbsent == null && (executorPutIfAbsent = this.childExecutors.putIfAbsent(childExecutorKey, (childExecutor = new ChildExecutor()))) == null) {
            executorPutIfAbsent = childExecutor;
        }
    ...
    final class ChildExecutor implements Runnable, Executor {
        ...
        public void run() {
            ...
            while (true) {
                ...
                boolean z = false;
                OrderedMemoryAwareThreadPoolExecutor.this.beforeExecute(threadCurrentThread, first);
                try {
                    try {
                        first.run();
                        z = true;
                        ...
                        if (!z) {
                            OrderedMemoryAwareThreadPoolExecutor.this.onAfterExecute(first, e);
                        }
                        throw e;
                    }
                } catch (Throwable th) {
