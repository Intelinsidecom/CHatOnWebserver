...
public class ChunkedWriteHandler implements ChannelDownstreamHandler, ChannelUpstreamHandler {
    ...
    private void discard(ChannelHandlerContext channelHandlerContext) {
        MessageEvent messageEventPoll;
        boolean z = false;
        ClosedChannelException closedChannelException = null;
        while (true) {
            MessageEvent messageEvent = this.currentEvent;
            if (this.currentEvent == null) {
                messageEventPoll = this.queue.poll();
            } else {
                this.currentEvent = null;
                messageEventPoll = messageEvent;
            }
            if (messageEventPoll == null) {
                break;
            }
            Object message = messageEventPoll.getMessage();
            ...
            messageEventPoll.getFuture().setFailure(closedChannelException2);
            z = true;
