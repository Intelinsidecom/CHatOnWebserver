...
class NioClientSocketPipelineSink extends AbstractChannelSink {
    ...
    /* renamed from: id */
    final int f19094id = nextId.incrementAndGet();
    ...
    NioClientSocketPipelineSink(Executor executor, Executor executor2, int i) {
        ...
        for (int i2 = 0; i2 < this.workers.length; i2++) {
            this.workers[i2] = new NioWorker(this.f19094id, i2 + 1, executor2);
        }
    }
            ...
            synchronized (this.startStopLock) {
                if (!this.started) {
                    try {
                        ...
                        try {
                            DeadLockProofWorker.start(NioClientSocketPipelineSink.this.bossExecutor, new ThreadRenamingRunnable(this, "New I/O client boss #" + NioClientSocketPipelineSink.this.f19094id));
                        } finally {
                        }
                    } catch (Throwable th) {
