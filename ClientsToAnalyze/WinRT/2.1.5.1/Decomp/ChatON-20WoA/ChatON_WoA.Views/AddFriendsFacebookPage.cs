...
public sealed class AddFriendsFacebookPage : Page, IComponentConnector
{
	private AddFriendsFacebookPageViewModel _viewModel;
	...
	public AddFriendsFacebookPage()
	{
		InitializeComponent();
		_viewModel = new AddFriendsFacebookPageViewModel(this);
	...
	private void PostMeButton_Click(object sender, RoutedEventArgs e)
	{
	}
	...
	private async void PostFriendButton_Click(object sender, RoutedEventArgs e)
	{
		...
		{
			new LoadingIndicator(null, (Panel)(object)RootFrame);
			FacebookServiceArgument result = await _viewModel.MakeFriendsList();
			LoadingIndicator.StopLoading();
			if (result.ResultType == FacebookServiceResultType.NETWORK_ERROR)
			{
			...
			else if (result.ResultType == FacebookServiceResultType.NO_RESULT)
			{
	...
	{
		if (await Utility.ShowOKCancleMessageDialog(Utility.GetResourceString("IDS_CHATON_POP_POST_CHATON_INVITATION_ON_FACEBOOK_Q")))
		{
			new LoadingIndicator(null, (Panel)(object)RootFrame, Utility.GetResourceString("IDS_CHATON_POP_POSTING_ING_FACEBOOK"));
			bool result = true;
			LoadingIndicator.StopLoading();
			if (result)
			{
				await Utility.ShowOKCancleMessageDialog(Utility.GetResourceString("IDS_CHATON_POP_CHATON_DOWNLOAD_LINK_HAS_BEEN_POSTED_ON_FRIENDS_WALL"), showCancel: false);
			}
			else
			{
	...
	private unsafe void dialog_SelectCompleted(object sender, object e)
	{
		...
		if ((bool)e)
		{
			new LoadingIndicator((Page)(object)this, (Panel)(object)RootFrame);
			...
			_webView.Navigate(FacebookService.GetLogoutUri());
		}
	}
	...
	{
		LoadingIndicator.StopLoading();
		if (e.Uri.OriginalString.Equals(FacebookService.RedirectUri))
		{
			((Page)this).Frame.GoBack();
		}
	}
	...
	public void InitializeComponent()
	{
		...
		{
			_contentLoaded = true;
			Application.LoadComponent((object)this, new Uri("ms-appx:///AddFriendsFacebookPage.xaml"), (ComponentResourceLocation)0);
