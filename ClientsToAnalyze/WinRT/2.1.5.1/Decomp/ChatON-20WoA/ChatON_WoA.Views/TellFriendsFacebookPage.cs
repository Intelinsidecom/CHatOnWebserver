...
public sealed class TellFriendsFacebookPage : Page, IComponentConnector
{
	private TellFriendsFacebookPageViewModel _viewModel;
	...
	public TellFriendsFacebookPage()
	{
		InitializeComponent();
		_viewModel = new TellFriendsFacebookPageViewModel(this);
	...
	private async void InviteButton_Click(object sender, RoutedEventArgs e)
	{
		if (await Utility.ShowOKCancleMessageDialog(Utility.GetResourceString("IDS_CHATON_POP_SHARE_CHATON_INVITATION_MESSAGE_Q"), showCancel: true, Utility.GetResourceString("IDS_CHATON_BUTTON_FACEBOOK")))
		{
			...
			bool result = await _viewModel.PostInvitationMessage(((FrameworkElement)((sender is Button) ? sender : null)).Tag.ToString());
			LoadingIndicator.StopLoading();
			if (result)
			{
	...
	public void InitializeComponent()
	{
		...
		{
			_contentLoaded = true;
			Application.LoadComponent((object)this, new Uri("ms-appx:///TellFriendsFacebookPage.xaml"), (ComponentResourceLocation)0);
