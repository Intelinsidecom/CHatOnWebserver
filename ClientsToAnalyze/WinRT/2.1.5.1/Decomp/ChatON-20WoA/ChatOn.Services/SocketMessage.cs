...
public class SocketMessage
{
	...
	public enum ResultCode
	{
		...
		Session_is_full = 3004,
		Session_user_not_exist = 3006,
		User_is_invalid = 4002,
		User_is_not_registered = 4005,
		Receiver_is_alreay_blocked = 5009,
		Requested_all_users_are_invalid = 5011,
		Message_over_limit = 5010,
		Receiver_is_Invalid = 6002,
		Invalid_paramater_length = 5005,
		Mapping_In_Progress = 7005
	}
	...
	public static byte[] AllowChatRequestMsg(string sender, string receiver, string uid, string imei, string sessionID, string key, ChatType chatType, long sessionMergedTimeStamp)
	{
		...
		builder.UID = uid;
		builder.IMEI = imei;
		builder.LastSessionMergeTimestamp = sessionMergedTimeStamp;
	...
	public static byte[] DeliveryChatReplyMsg(IList<InboxMessage> messages, string uid, string key, string myNumber)
	{
		...
		foreach (InboxMessage message in messages)
		{
			...
			string text = myNumber;
			if (sender == text && message.MsgType == MsgType.ANS)
			{
				text = message.GetReceivers(0);
			}
			DeliveryChatReplyItem.Builder builder2 = new DeliveryChatReplyItem.Builder();
			builder2.Sender = sender;
			builder2.Receiver = text;
	...
	public static byte[] IsTypingRequestMsg(string sender, string sessionId, ChatType chatType, ContentType contentType, string uid, string key, string typingStatus = "idle", int refresh = 30)
	{
		IsTyping.Builder builder = new IsTyping.Builder();
		builder.ChatType = chatType;
		builder.ContentType = contentType;
		builder.Refresh = refresh;
		builder.Sender = sender;
		builder.SessionID = sessionId;
		builder.State = typingStatus;
		IsTyping isTyping = builder.Build();
		return EncryptAndCombineHead(key, uid, 24, isTyping.ToByteArray());
	}
