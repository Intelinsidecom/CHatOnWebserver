...
public sealed class FieldDescriptor : IndexedDescriptorBase<FieldDescriptorProto, FieldOptions>, IComparable<FieldDescriptor>, IFieldDescriptorLite, IComparable<IFieldDescriptorLite>
{
	...
	IEnumLiteMap IFieldDescriptorLite.EnumType => EnumType;

	bool IFieldDescriptorLite.MessageSetWireFormat => ContainingType.Options.MessageSetWireFormat;
	...
	private static IDictionary<FieldType, MappedType> MapFieldTypes()
	{
		...
		foreach (FieldInfo declaredField in typeof(FieldType).GetTypeInfo().DeclaredFields)
		{
			...
			FieldMappingAttribute fieldMappingAttribute = (FieldMappingAttribute)list[0];
