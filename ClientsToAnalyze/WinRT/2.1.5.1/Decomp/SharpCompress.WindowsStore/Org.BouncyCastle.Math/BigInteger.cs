...
public class BigInteger
{
	...
	public BigInteger ModPow(BigInteger exponent, BigInteger m)
	{
		...
		BigInteger bigInteger3 = new BigInteger(1, array4, checkMag: true);
		if (exponent.sign <= 0)
		{
	...
	private static void MultiplyMonty(int[] a, int[] x, int[] y, int[] m, long mQuote)
	{
		...
		for (int num4 = num; num4 > 0; num4--)
		{
			...
			long num10 = (num7 >>> 32) + (num8 >>> 32) + (num9 >>> 32);
			for (int num11 = num2; num11 > 0; num11--)
			{
				...
				num10 = (num10 >>> 32) + (num7 >>> 32) + (num8 >>> 32) + (num9 >>> 32);
				a[num11 + 1] = (int)num9;
			}
	...
	public BigInteger Multiply(BigInteger val)
	{
		...
		return new BigInteger(sign * val.sign, array, checkMag: true);
	}
	...
	public string ToString(int radix)
	{
		...
		case 2:
		{
			...
			for (int num2 = BitLength - 2; num2 >= 0; num2--)
			{
				stringBuilder.Append(TestBit(num2) ? '1' : '0');
			}
			break;
		}
		default:
		{
			...
			while (bigInteger2.sign != 0)
			{
				...
				{
					list.Add(bigInteger3.magnitude[0].ToString("d"));
				}
