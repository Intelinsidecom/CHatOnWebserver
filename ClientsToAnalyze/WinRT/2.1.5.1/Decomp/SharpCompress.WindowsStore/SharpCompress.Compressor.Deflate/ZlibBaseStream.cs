...
internal class ZlibBaseStream : Stream
{
	...
	private void finish()
	{
		...
		if (_streamMode == StreamMode.Writer)
		{
			...
			do
			{
				...
				if (num != 1 && num != 0)
				{
					...
					if (_z.Message == null)
					{
						throw new ZlibException(string.Format("{0}: (rc = {1})", new object[2] { text, num }));
					}
		...
		else
		{
			...
			{
				throw new ZlibException(string.Format("Bad CRC32 in GZIP stream. (actual({0:X8})!=expected({1:X8}))", new object[2] { crc32Result2, num4 }));
			}
			if (num6 != num5)
			{
				throw new ZlibException(string.Format("Bad size in GZIP stream. (actual({0})!=expected({1}))", new object[2] { num6, num5 }));
			}
		}
	}
	...
	public override int Read(byte[] buffer, int offset, int count)
	{
		...
		do
		{
			...
			if (num != 0 && num != 1)
			{
				throw new ZlibException(string.Format("{0}flating:  rc={1}  msg={2}", new object[3]
				{
					_wantCompress ? "de" : "in",
					num,
					_z.Message
		...
		if (_z.AvailableBytesOut > 0)
		{
			...
			if (nomoreinput && _wantCompress)
			{
				...
				if (num != 0 && num != 1)
				{
					throw new ZlibException(string.Format("Deflating:  rc={0}  msg={1}", new object[2] { num, _z.Message }));
				}
			}
		}
