...
public abstract class AbstractMessage<TMessage, TBuilder> : IMessage<TMessage, TBuilder>, IMessage<TMessage>, IMessage where TMessage : AbstractMessage<TMessage, TBuilder> where TBuilder : AbstractBuilder<TMessage, TBuilder>
{
	...
	public virtual bool IsInitialized
	{
		get
		{
			...
			foreach (KeyValuePair<FieldDescriptor, object> allField in AllFields)
			{
				...
				if (key.IsRepeated)
				{
					foreach (IMessage item in (IEnumerable)allField.Value)
					{
				...
				else if (!((IMessage)allField.Value).IsInitialized)
				{
					return false;
				}
			}
