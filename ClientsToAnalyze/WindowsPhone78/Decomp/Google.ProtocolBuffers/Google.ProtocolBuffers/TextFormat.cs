...
public static class TextFormat
{
	...
	private static long ParseInteger(string text, bool isSigned, bool isLong)
	{
		...
		{
			string text3 = string.Format("{0}-bit {1}signed integer", isLong ? 64 : 32, isSigned ? "" : "un");
		...
		if (num2 > num4)
		{
			string text5 = string.Format("{0}-bit {1}signed integer", isLong ? 64 : 32, isSigned ? "" : "un");
	...
	private static void MergeField(TextTokenizer tokenizer, ExtensionRegistry extensionRegistry, IBuilder builder)
	{
		...
		else
		{
			...
			{
				string name = text.ToLower(CultureInfo.InvariantCulture);
		...
		if (fieldDescriptor.MappedType == MappedType.Message)
		{
			...
			IBuilder builder2 = ((extensionInfo != null) ? extensionInfo.DefaultInstance.WeakCreateBuilderForType() : builder.CreateBuilderForField(fieldDescriptor));
